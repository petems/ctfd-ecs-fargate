output "sns_topic_arn" {
  ...
  value       = var.create_sns_topic && var.create_dashboard && length(aws_sns_topic.alerts) > 0 ? aws_sns_topic.alerts[0].arn : null
}

output "sns_topic_name" {
  description = "Name of the SNS topic for alerts"
  value       = var.create_sns_topic && var.create_dashboard && length(aws_sns_topic.alerts) > 0 ? aws_sns_topic.alerts[0].name : null
}
